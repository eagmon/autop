...
# blank  = zeros(T, fieldRes, fieldRes)
...
wFile = matopen("data/w3f1.mat")
Wfield = MAT.read(wFile, "Wfield")

aFile = matopen("data/a3f1.mat")
Afield = MAT.read(aFile, "Afield")

mFile = matopen("data/m3f1.mat")
Mfield = MAT.read(mFile, "Mfield")

fFile = matopen("data/f3f1.mat")
Ffield = MAT.read(fFile, "Ffield")

dFile = matopen("data/d3f1.mat")
directionfield = MAT.read(dFile, "directionfield")
...
while (t <= timeTotal) ##&& (meanMField < 2) && (meanMField > 0.001) && (meanAField < 2) && (meanAField > 0.001)
    ...
    if (t == tearTime1)
        tS = iround(fieldRes/2+tearSize1*fieldRes/fieldSize-2)
        t0 = iround(fieldRes/2-2)
        Mfield[fieldRes/2+4:fieldRes/2+13,t0:tS] = 0 #directionfield[fieldRes/2:fieldRes,t0:tS]=0
    end

    if (t == tearTime2)
        tS = iround(fieldRes/2+tearSize1*fieldRes/fieldSize-2)
        t0 = iround(fieldRes/2-2)
        directionfield[fieldRes/2+4:fieldRes/2+13,t0:tS] = pi .* rand(10,tS-t0+1) #directionfield[fieldRes/2:fieldRes,t0:tS] = pi .* rand(iround(fieldRes-fieldRes/2+1),tS-t0+1)
    end
